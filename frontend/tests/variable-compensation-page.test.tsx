...
function result(status: "calculated" | "incomplete" = "calculated", total = "975.00") {
  return { competence_month: "2026-08", collaborator_count: 1, calculated_count: status === "calculated" ? 1 : 0, incomplete_count: status === "incomplete" ? 1 : 0, projected_total: status === "calculated" ? total : null, items: [{ collaborator_id: "operator-1", display_name: "operator-1", status, pending_reasons: status === "incomplete" ? ["recurrence_incomplete"] : [], csat: { status: "eligible", reference_month: "2026-08", eligible: true, tier: "gold", amount: "800.00", individual_value: "9.60", team_average: "9.20", modality: "chat", response_rate: "0.50", minimum_response_rate: "0.40" }, recurrence: { status: "eligible", reference_month: "2026-07", eligible: true, tier: "silver", amount: "200.00", individual_value: "0.08", team_average: "0.14", team_average_cap_passed: true }, delays: { count: 2, amount: "-25.00" }, absences: { count: 0, amount: "0.00" }, positive_amount: status === "calculated" ? "1000.00" : null, deductions_amount: status === "calculated" ? "-25.00" : null, total_amount: status === "calculated" ? total : null }] } as const;
}
...
test("renders negative total, loading, empty and error states", async () => {
  ...
  getClosure.mockResolvedValueOnce({ ...result(), collaborator_count: 0, calculated_count: 0, items: [], projected_total: "0.00" }); const empty = page();
